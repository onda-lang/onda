...
pub(crate) fn classify_def_indexed_binding(
    base: &str,
    local_array_aliases: &HashMap<String, LocalArrayAliasInfo>,
    ...
    if let Some((root, field)) = split_field_path(base, errors) {
        if let Some(struct_name) = param_structs.get(root) {
            if let Some(kind) = classify_named_or_flattened_field(struct_name, field, struct_defs) {
                return Some(kind);
            }
        }
    } else if let Some(self_struct) = param_structs.get("self") {
        if !struct_defs.contains_key(self_struct) {
            return Some(IndexedBindingKind::ProcArrayAlias);
        }
        if let Some(kind) = classify_named_or_flattened_field(self_struct, base, struct_defs) {
            return Some(kind);
        }
    }

    let prefix = format!("{base}[");
    if state_scalars.keys().any(|field| field.starts_with(&prefix)) {
        return Some(IndexedBindingKind::ProcArrayAlias);
    }
