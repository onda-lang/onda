...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=ONDA_LLVM_LINK_MODE");
...
fn detect_link_mode(prefix_path: &Path) -> LinkMode {
    let requested = env::var("ONDA_LLVM_LINK_MODE")
        .ok()
    ...
    match requested.as_deref() {
        ...
        Some("shared") | Some("dynamic") | Some("dylib") => {
            if has_shared_llvm_c(prefix_path) {
                ...
                panic!(
                    "ONDA_LLVM_LINK_MODE=shared requested, but no shared LLVM-C runtime found under '{}'.",
                    prefix_path.display()
                );
            }
        }
        ...
        Some(other) => {
            panic!("Unsupported ONDA_LLVM_LINK_MODE='{other}'. Use auto|shared|static.");
        }
    }
}
