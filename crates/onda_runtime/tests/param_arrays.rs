...
fn event_array_reads_preserve_process_snapshot_across_segments() {
    for read in [
        ...
        trigger_event_by_index(&mut instance, 0, &[], ExecutionOutput::none()).unwrap();
        process_checked_segment(
            &mut instance,
            2,
            2,
        ...
        process_checked_segment(
            ...
            0,
            4,
            PROCESS_BEGIN_BLOCK | PROCESS_END_BLOCK,
            ExecutionOutput::none(),
        )
        .unwrap();
...
"#,
    ...
    for (event, expected) in [(None, 40.0), (Some(0), 0.0), (Some(1), 4.0)] {
        if let Some(event) = event {
            trigger_event_by_index(&mut instance, event, &[], ExecutionOutput::none()).unwrap();
        }
        process_checked_segment(
            &mut instance,
            0,
