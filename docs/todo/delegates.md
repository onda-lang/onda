...
event and a delegate in the same owner cannot have the same name. This keeps `owner.name` resolution
and host metadata unambiguous.
...
Emission is not allowed from `init`. Instance creation and reset can happen before a host outbox is
ready, and reset should not unexpectedly produce external behavior. A runtime def that may emit a
delegate therefore cannot be called from `init`; this becomes part of its inferred effect.
...
| Block prelude | frame boundary `0` |
...
arguments and do not introduce a hidden runtime cursor. A block-post occurrence lies on the end
boundary and may therefore have offset `block_size`. Host adapters are responsible for translating
that boundary into their native event-time representation.
...
2. Add semantic symbols, payload checking, owner access control, init-effect rejection, static
   subscription resolution, and cross-event cycle diagnostics.
3. Lower proc delegates and subscriptions to direct internal calls. Test nested procs, constant
   proc-array elements, fanout ordering, payload transformation, and dead unobserved emissions
   before adding a host boundary.
4. Add top-level delegate descriptors and `EmitDelegate` to MIR, including validation, deterministic
   dumps, JSON/MessagePack transport, optimization effects, and backend parity fixtures.
5. Add a fixed-payload realtime outbox to native runtime/codegen, the C API, processor metadata,
   AOT artifacts, Binaryen, and the Web Audio adapter.
...
events, defs, and subscriptions; multiple same-frame emissions; segmented and zero-frame calls;
oversampled emission ordering; missing outboxes; exact-fit and overflowing outboxes; reset behavior;
fixed arrays and slices; snapshots; native/Binaryen parity; and all invalid ownership and cycle
forms.
